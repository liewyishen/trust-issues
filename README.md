...
build`, on one date.** There is CI now, and it does not reach this paragraph: it runs
ruff, mypy, the test suite and the frontend build, and builds no image. Nothing rebuilds
the image on a schedule, nothing re-checks these five numbers, and **nothing will go red
when one of them stops being true.** A measurement is not a guard, and a dated measurement
described as though it were coverage is the failure this file's neighbours were written to
catch. The 936MB is that build; the **937MB** above is a different build, from Phase 2.
Both were measured, nothing has reconciled them, and this sentence does not guess which is
now right.
...
| **Ruff** | Lint, and **deliberately not format**. `ruff format` would reflow hand-aligned literals — `emp_order`'s 0–10 ladder collapses into one entry per line — and rewrite most of the repo for a uniformity that costs the git blame of files whose history *is* their documentation. The measurement behind that call, and the rule families kept off because they would come for deliberate constructions (`/drift`'s in-handler import, `SELECTED_THRESHOLD`'s exact float), are recorded once in `pyproject.toml`'s `[tool.ruff]` comment rather than repeated here. It is enforced now: the `ruff` job runs `ruff check .` on every pull request and push to `main`, and the branch-protection rule below will not let `main` take it red. This row used to point at the image numbers above as the same case; it no longer is, because those five still have nothing rechecking them |
| **mypy** | Non-strict type check over `src/` `serving/` `pipelines/` `scripts/`. Where a runtime `raise` already enforces a contract the checker cannot see — `/fairness`'s 404 guard, `explain_applicants()`'s all-or-none escape hatch — the annotation stays honest about the unguarded type and the call carries a `type: ignore` with its error code and a reason. Never an `assert` or a `cast`: both would restate the guarantee in a place that checks nothing, and the `raise` is the thing that actually fails closed. Libraries that ship no types are silenced per module, not globally. Enforced the same way as the linter, in its own job |
| **GitHub Actions** | The only thing here that runs without being asked. Four parallel jobs on pull requests to `main` and pushes to it. Three of them — `ruff`, `mypy`, `pytest` — run the same three commands listed under Setup, on Python 3.11; the fourth, `frontend`, runs `npm run build` on Node 24, which type checks (`tsc -b`) and bundles (`vite build`) the UI in one command, and which needs the Python environment as well, because that build regenerates the enum file from the real Python value sets before it compiles. It covers both toolchains now, but neither of them completely: `frontend/` is built and type checked but not linted — oxlint is in its devDependencies, `.oxlintrc.json` configures it, and it passes today, but no npm script calls it, so nothing here can — and training, the fairness audit and the drift check never run, because all three read the 167 MB dataset this repo deliberately does not carry. On a clean checkout thirteen tests skip for want of `models/*.pkl`, so the run prints its collected count and every skip reason — "green" and "green with thirteen tests that never ran" are different claims. Why the suite runs with no selection flags, and why the network guard is left free to fire here, are argued in [`.github/workflows/ci.yml`](.github/workflows/ci.yml) rather than repeated. **A branch-protection rule on `main` requires all four green before any merge, administrators included.** That rule is a GitHub repository setting rather than a file, which makes it the one claim on this page you cannot check by reading the repo: `pyproject.toml` can vouch for what ruff and mypy do, and nothing here can vouch for the gate |
