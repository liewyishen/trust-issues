 ...
 *
 * This is TIER 3. It sits between the decision (tier 2) and the additive
 * explanation (tier 4) because it is what connects them: tier 2 shows a
 * probability, tier 4 shows contributions in log-odds, and a reader who reads
 * those in sequence is entitled to ask why the contributions are not in
 * probability points. The answer is this shape. On a flat block the derivative of
 * p_cal with respect to the model's own output is exactly zero, so "this feature
 * is worth N points of probability" has no value to compute -- not a value nobody
 * has computed yet, no value at all.
 *
 ...
 * with. There is no snapshot in this file. Retrain the calibrator and this
 * picture changes, which is the only way it can keep being true.
 */
...
export function CalibratorExplainer({
  ...
  // The probe starts ON the applicant, so the first thing a reader sees is their
  // own point; dragging is what separates the two.
  const [probeRaw, setProbeRaw] = React.useState(r.p_raw)
  React.useEffect(() => setProbeRaw(r.p_raw), [r.p_raw])
  ...
  // The applicant's own point is NEVER recomputed -- it is the API's p_raw and
  // the API's p_calibrated, plotted. This reading exists only to CHECK that the
  // curve being drawn is the curve that produced the decision above: same idiom
  // as tier 4's additivity sum. It can only pass; it is here to be checked.
  const check = evaluate(cal, r.p_raw)
  const reproduces = Math.abs(check.p_cal - r.p_calibrated) < 1e-12
  ...
  const applicantColour =
    r.decision === "APPROVE" ? "var(--color-approve)" : "var(--color-reject)"
  const probeColour =
    probeDecision === "APPROVE" ? "var(--color-approve)" : "var(--color-reject)"

  return (
    <Card>
      <CardHeader className="flex items-center justify-between">
        <CardTitle>Why that probability, and not a nearby one</CardTitle>
      ...
      <CardContent className="space-y-4">
        <p className="text-[12px] leading-relaxed text-muted">
          The <span className="text-fg">{pct(r.p_calibrated)}</span> above is not a point on a
          smooth curve. It is one of{" "}
          <span className="text-fg">{cal.n_distinct_y} reachable values</span> — the output of
          the shipped isotonic calibrator, drawn below exactly as it is: {cal.n_distinct_y} flat
          blocks joined by {cal.n_distinct_y - 1} near-vertical ramps. Your applicant sits on
          one of those blocks.
        </p>
        ...
        <figure className="rounded-md border border-border bg-surface-2/30 p-2">
          ...
          >
            ...
            <text
              x={M.l + 6} y={sy(yMax) - 5}
              fontSize={9.5} fill="var(--color-faint)"
            >
            ...
            {[0, 0.2, 0.4, 0.6, 0.8, 1].map((t) => (
              <g key={`x${t}`}>
                ...
                />
                <text
                  x={sx(t)} y={M.t + IN.h + 14}
                  textAnchor="middle" fontSize={10} fill="var(--color-faint)"
                  className="tnum"
                >
                  {t.toFixed(1)}
                </text>
            ...
            {[0, 0.2, 0.4, 0.6, 0.8, 1].map((t) => (
              <g key={`y${t}`}>
                ...
                <text
                  x={M.l - 8} y={sy(t) + 3.5}
                  textAnchor="end" fontSize={10} fill="var(--color-faint)"
                  className="tnum"
                >
                  {t.toFixed(1)}
                </text>
            ...
            />
            <text
              x={M.l + IN.w - 4} y={sy(cal.threshold) - 5}
              textAnchor="end" fontSize={9.5} fill="var(--color-reject)" opacity={0.85}
            >
            ...
            {/* --- the probe: what the reader is dragging --- */}
            <line
            ...
            {/* --- the applicant: the anchor. Read off the API response, never
                    recomputed. Stays visible while the probe moves, so "where I
                    actually am" and "what I am dragging" are both on screen. --- */}
            <g>
              <circle
                cx={sx(r.p_raw)} cy={sy(r.p_calibrated)} r={7}
                fill="none" stroke={applicantColour} strokeWidth={1.75}
              />
              <circle cx={sx(r.p_raw)} cy={sy(r.p_calibrated)} r={1.75} fill={applicantColour} />
              <text
                x={sx(r.p_raw) + 12} y={sy(r.p_calibrated) - 8}
                fontSize={10} fill={applicantColour}
              >
                this applicant · {pct(r.p_calibrated)}
              </text>
            </g>
          </svg>
        </figure>
        ...
        <div className="space-y-2.5">
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => setProbeRaw(r.p_raw)}
              title="Return the probe to this applicant"
        ...
        <div className="space-y-2 border-t border-border pt-3">
          <p className="text-[11px] leading-relaxed text-faint">
            ...
            <span className="text-muted">
              not a rounding artifact, but the shipped function
            </span>
            . That is also why the contributions in the panel below are given in log-odds and
            never converted to probability points: on a flat block{" "}
            <code className="text-muted">dp_cal/dp_raw</code> is exactly 0, so there is no
            quantity to convert. <code className="text-muted">docs/explainability.md</code>{" "}
            measures the flat fraction of the reject region at{" "}
            <span className="text-muted">99.31%</span> of the p_raw axis — a statement about
            the axis, not about how many applicants fall on it.
          </p>
            ...
            {reproduces ? (
              <>
                ...
                <span>
                  ...
                  <code className="text-muted">GET /calibrator</code>'s knots, read at this
                  applicant's own <code className="text-muted">p_raw</code>, reproduce the{" "}
                  <code className="text-muted">p_calibrated</code> the API returned — to
                  within 1e-12. Nothing here is a snapshot; retrain the calibrator and this
                  plot changes with it.
                </span>
              </>
            ) : (
              <span>
                The plotted curve does <em>not</em> reproduce the API's own p_calibrated at
                this applicant's p_raw. The drawing and the decision disagree — trust the
                decision, not this plot.
              </span>
            )}
          </div>
        </div>
      ...
      <p className="mt-2.5 border-t border-border pt-2 text-[11px] leading-relaxed text-muted">
        ...
        {probe.region === "ramp" && (
          <>
            ...
            narrower than a single pixel of the plot above, so you could not have dragged onto
            it by accident. That impossibility <em>is</em> the finding. The whole jump from{" "}
            <span className="tnum">{pct(probe.yspan[0])}</span> to{" "}
            <span className="tnum">{pct(probe.yspan[1])}</span> happens across{" "}
            <span className="tnum text-fg">{(width * 1e6).toFixed(1)} parts per million</span>{" "}
            of p_raw, at a slope of <span className="tnum">{probe.slope.toFixed(1)}</span>.
            {flipsDecision && (
              <>
                {" "}
        ...
        {probe.region === "clipped-high" && (
          <>
            ...
            <span className="text-fg">{((1 - cal.x_max) * 100).toFixed(1)}% of the raw axis</span>{" "}
