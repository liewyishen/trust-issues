...
ruff, mypy and the test suite, and builds nothing. Nothing rebuilds the image on a
...
| **GitHub Actions** | The only thing here that runs without being asked. Three parallel jobs — `ruff`, `mypy`, `pytest` — on pull requests to `main` and pushes to it, running the same three commands listed under Setup, on Python 3.11 only. It covers the Python toolchain and nothing else: `frontend/` is unchecked, and training, the fairness audit and the drift check never run, because all three read the 167 MB dataset this repo deliberately does not carry. On a clean checkout thirteen tests skip for want of `models/*.pkl`, so the run prints its collected count and every skip reason — "green" and "green with thirteen tests that never ran" are different claims. Why the suite runs with no selection flags, and why the network guard is left free to fire here, are argued in [`.github/workflows/ci.yml`](.github/workflows/ci.yml) rather than repeated. **A branch-protection rule on `main` requires all three green before any merge, administrators included.** That rule is a GitHub repository setting rather than a file, which makes it the one claim on this page you cannot check by reading the repo: `pyproject.toml` can vouch for what ruff and mypy do, and nothing here can vouch for the gate |
